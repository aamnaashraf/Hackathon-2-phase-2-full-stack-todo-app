...
interface AddTodoModalProps {
  onClose: () => void
  onAdd: (title: string, description?: string, due_date?: string, priority?: 'low' | 'medium' | 'high') => void
  loading: boolean
}
...
export default function AddTodoModal({ onClose, onAdd, loading }: AddTodoModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd(title, description, dueDate || undefined, priority)
  }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md transform transition-all">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Add New Todo</h3>
            <button
              onClick={onClose}
